...
const Header = (props: Props) => {
  ...
  const { title, header, children, medicine } = props
  const [loaded, setLoaded] = useState(false)

  useEffect(() => {
    // $("#loading-start").css({ top: 0 });
    // if (loaded === false) {
    //   $(document).ready(() => {
    //     setTimeout(() => {
    //       setLoaded(true);
    //       $("#loading-start").animate({ opacity: 0 }, 500, () => {
    //         $("#loading-start").css({ display: "none" });
    //       });
    //     }, 100);
    //   });
    //   // For FB chat
    //   const script = document.createElement("script");
    //   script.src = "./../static/javascripts/fb_chat.js";
    //   script.async = true;
    //   document.body.appendChild(script);
    // } else $("#loading-start").css({ display: "none" });
  }, [])
